    ...
    public partial class LoginPage : ContentPage
    {
		...
		public async void GetJSON2(string username, string password)
		{
			...
			else
			{
				...
				Application.Current.Properties["datenow"] = date;
				//check login//
                var response = await client.GetAsync("http://hotelsoftware.in.th/Webrestful/api/HotelAuthen/VerifyUserNamePassword?szHotelDB="+database+"&szIPServer=genius-ihotel5.cloudapp.net&szUserLogin="+username+"&szPassword="+password+"&szDeviceCode=1234");
				string contactsJson = response.Content.ReadAsStringAsync().Result;
